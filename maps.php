...
<head>
    ...
    <title><?php echo $t['page_title_maps']; ?></title>
...
<body>
    ...
    <div class="content">
        <h1 class="page-title"><?php echo $t['page_title_maps']; ?></h1>
        
        <div class="maps-filter">
            <form action="maps.php" method="get" id="searchForm">
                <input type="text" id="mapSearch" name="search" placeholder="<?php echo htmlspecialchars($t['search_placeholder']); ?>" class="search-input" value="<?php echo htmlspecialchars($searchTerm); ?>">
                <input type="hidden" name="filter" id="difficultyFilter" value="<?php echo htmlspecialchars($difficultyFilter); ?>">
                <div class="filter-buttons">
                    <button type="button" class="filter-btn <?php echo $difficultyFilter === 'all' ? 'active' : ''; ?>" data-filter="all"><?php echo $t['filter_all']; ?></button>
                    <button type="button" class="filter-btn <?php echo $difficultyFilter === 'easy' ? 'active' : ''; ?>" data-filter="easy"><?php echo $t['filter_easy']; ?></button>
                    <button type="button" class="filter-btn <?php echo $difficultyFilter === 'medium' ? 'active' : ''; ?>" data-filter="medium"><?php echo $t['filter_medium']; ?></button>
                    <button type="button" class="filter-btn <?php echo $difficultyFilter === 'hard' ? 'active' : ''; ?>" data-filter="hard"><?php echo $t['filter_hard']; ?></button>
                </div>
            </form>
        </div>
                ...
                <div class="map-card-content">
                    ...
                    <div class="map-info">
                        ...
                        <div class="map-author">
                            <?php echo htmlspecialchars($mapInfo['author'] ?? $t['unknown']); ?>
                        </div>
                    </div>
                    <a href="map_details.php?map=<?php echo urlencode($mapName); ?>" class="map-details-btn"><?php echo $t['see_details']; ?></a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="no-maps">
            ...
            <a href="maps.php" class="back-button"><?php echo $t['reset_filters']; ?></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        ...
        <div class="pagination">
            ...
            <div class="page-info"><?php echo $t['page_word']; ?> <?php echo $currentPage; ?> <?php echo $t['page_of']; ?> <?php echo $totalPages; ?> (<?php echo $totalMaps; ?> <?php echo $t['maps_word']; ?>)</div>
        </div>
        <?php endif; ?>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (isMobile) {
            // Sur mobile, utiliser un bouton de recherche explicite ou "blur" (perte de focus)
            ...
            searchButton.innerHTML = '<?php echo $t['search_button']; ?>';
            searchForm.appendChild(searchButton);
        } else {
            // Sur desktop, garder le comportement actuel avec délai
            searchInput.addEventListener('input', function() {
                clearTimeout(searchTimeout);
                searchTimeout = setTimeout(function() {
                    searchForm.submit();
                }, 1600); // Soumettre après 500ms d'inactivité
            });
        }
